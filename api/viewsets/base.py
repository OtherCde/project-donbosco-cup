...
from api.permissions.base import IsCRUDOrReadOnlyUser, IsAdminOrCRUDUser
from api.serializers.base import (
    TournamentSerializer, TournamentCategorySerializer, PhaseSerializer, RoundSerializer,
    TeamSerializer, PlayerSerializer, MatchSerializer, MatchTeamSerializer,
    MatchEventSerializer, UserSerializer, TeamWithPlayersSerializer,
...
class TournamentViewSet(viewsets.ModelViewSet):
    ...
    def current(self, request):
        ...
        if current_tournament:
            ...
            return Response(serializer.data)
        return Response({'message': 'No hay torneo actual'}, status=status.HTTP_404_NOT_FOUND)
...
class TeamViewSet(viewsets.ModelViewSet):
    ...
    def by_tournament(self, request):
        ...
        if tournament_id:
            teams = Team.objects.filter(tournament_category__tournament_id=tournament_id)
            serializer = self.get_serializer(teams, many=True)
            return Response(serializer.data)
        return Response({'error': 'tournament_id es requerido'}, status=status.HTTP_400_BAD_REQUEST)
...
class PlayerViewSet(viewsets.ModelViewSet):
    ...
    permission_classes = [IsCRUDOrReadOnlyUser]
    filter_backends = [DjangoFilterBackend, filters.SearchFilter, filters.OrderingFilter]
    filterset_fields = ['team', 'position', 'promo', 'oficio', 'team__tournament_category']
    search_fields = ['first_name', 'last_name', 'dni', 'jersey_number', 'telefono', 'oficio']
    ...
    def by_position(self, request):
        ...
        if position:
            ...
            serializer = self.get_serializer(players, many=True)
            return Response(serializer.data)
        return Response({'error': 'position es requerido'}, status=status.HTTP_400_BAD_REQUEST)
...
class MatchViewSet(viewsets.ModelViewSet):
    ...
    def by_date(self, request):
        ...
        if date:
            ...
            return Response(serializer.data)
        return Response({'error': 'date es requerido'}, status=status.HTTP_400_BAD_REQUEST)
...
class MatchEventViewSet(viewsets.ModelViewSet):
    ...
    def by_match(self, request):
        ...
        if match_id:
            ...
            serializer = self.get_serializer(events, many=True)
            return Response(serializer.data)
        return Response({'error': 'match_id es requerido'}, status=status.HTTP_400_BAD_REQUEST)
