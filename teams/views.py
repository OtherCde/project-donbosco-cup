...
from .utils import extract_players_from_excel, auto_assign_jersey_numbers, auto_assign_dni
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                if not players_data:
                    messages.warning(request, "No se encontraron jugadores en el archivo Excel.")
                ...
                for player_data in players_data:
                    try:
                        ...
                        if Player.objects.filter(team=team, dni=player_data['dni']).exists():
                            skipped_players.append(f"{player_data['first_name']} {player_data['last_name']} (DNI ya existe)")
                            continue
                        
                        # Verificar si ya existe un jugador con el mismo número de camiseta
                        if Player.objects.filter(team=team, jersey_number=player_data['jersey_number']).exists():
                            errors.append(f"Número de camiseta {player_data['jersey_number']} ya está en uso para {player_data['first_name']} {player_data['last_name']}")
                            continue
                        ...
                        errors.append(f"Error creando jugador {player_data['first_name']} {player_data['last_name']} (fila {player_data.get('row_number', '?')}): {str(e)}")
                
                # Mostrar resultados
                if created_players:
                    messages.success(request, f"✅ Se crearon {len(created_players)} jugadores exitosamente.")
                
                if skipped_players:
                    messages.info(request, f"⚠️ Se omitieron {len(skipped_players)} jugadores que ya existían.")
