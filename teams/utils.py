...
def extract_players_from_excel(excel_file, header_row=24, start_data_row=25, default_position='MID'):
    """
...
def find_column_mapping(headers):
    ...
    for col, header in headers.items():
        ...
        elif any(word in header for word in ['FECHA', 'NAC', 'BIRTH', 'BORN', 'NACIMIENTO']):
            mapping['fecha_nacimiento'] = col
            
        # Mapear número de jugador
        elif header.isdigit() or any(word in header for word in ['NUM', 'NUMERO', 'NUMBER', '#']):
...
def extract_player_from_row(worksheet, row, column_mapping, default_position):
    ...
    if 'fecha_nacimiento' in column_mapping:
        fecha_nac = worksheet.cell(row=row, column=column_mapping['fecha_nacimiento']).value
        if fecha_nac:
